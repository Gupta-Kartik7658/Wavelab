...
st.set_page_config(page_title="Bivariate Laplacian Distribution", initial_sidebar_state="collapsed", page_icon="Kartik/logo (1).png", layout="wide")
...
with col2:
    ...
    st.markdown(f"""<br>""", unsafe_allow_html=True)
    bx = st.slider('Scale Perimeter ($b_x$)', 0.1, 10.0, 1.0) 
    st.markdown(f"""<br>""", unsafe_allow_html=True)
    by = st.slider('Scale Perimeter ($b_y$)', 0.1, 10.0, 1.0) 
