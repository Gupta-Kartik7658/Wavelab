...
def bivariate_normal_distribution(col1, col2, mx=3, my=5, bx=0.25, by=0.5, xlim=5, ylim=5, grid_points=100):
    ...
    X = np.abs(x - mx) / bx
    Y = np.abs(y - my) / by
    fxy = (1 / (2 * bx * by)) * np.exp((X + Y))

    # Calculate CDF values
    def integrand(x,y):
        X = (x - mx) / bx
        Y = (y - my) / by
        return (1 / (2 * np.pi * bx * by)) * np.exp(-0.5 * (X**2 + Y**2))
...
with col2:
    ...
    bx = st.slider('Standard Deviation ($\sigma_x$)', 0.1, 10.0, 1.0) 
    by = st.slider('Standard Deviation ($\sigma_y$)', 0.1, 10.0, 1.0) 
...
col1, col2 = st.columns(2)
bivariate_normal_distribution(col1, col2, mux, muy, bx, by, xlim, ylim, grid_points)
